...
export async function apiFetch(path: string, options: RequestInit = {}) {
    ...
    if (!response.ok) {
        ...
        const message = (errorData.detail && typeof errorData.detail === 'string')
            ? errorData.detail
...
export const fetchSessions = async ({ pageParam = 1 }: { pageParam?: number }): Promise<SessionsPage> => {
    return apiFetch(`/session/user/sessions?page=${pageParam}&items_per_page=8`);
};

export const startSession = (data: { session_id: string, name: string, data_context: string }) =>
    apiFetch('/session/start', { method: 'POST', body: JSON.stringify(data) })

export const fetchSessionById = (sessionId: string) => apiFetch(`/session/${sessionId}`)

export const deleteSession = (sessionId: string) =>
    apiFetch(`/session/${sessionId}`, { method: 'DELETE' });

export const duplicateSession = (sessionId: string, newName: string, copyOptions: object) =>
...
export const clarifySession = (sessionId: string, clarifications: Record<string, string>) =>
    apiFetch(`/session/clarify/${sessionId}`, {
        method: 'POST',
        body: JSON.stringify({ clarifications })
    });

export const createCheckpoint = ({ sessionId, label }: { sessionId: string; label: string }) =>
    apiFetch(`/session/checkpoint/${sessionId}/checkpoint?label=${encodeURIComponent(label)}`, { method: 'POST' });

export const fetchCheckpoints = (sessionId: string) =>
    apiFetch(`/session/checkpoint/${sessionId}/checkpoints?page=1&items_per_page=100`)

export const activateCheckpoint = ({ sessionId, checkpointId }: { sessionId: string; checkpointId: string }) =>
    apiFetch(`/session/checkpoint/${sessionId}/checkpoint/activate/${checkpointId}`, { method: 'POST' })

export const deactivateCheckpoint = (sessionId: string) =>
    apiFetch(`/session/checkpoint/${sessionId}/checkpoint/deactivate`, { method: 'POST' })

export const deleteCheckpoint = (checkpointId: string) =>
    apiFetch(`/session/checkpoint/checkpoint/${checkpointId}`, { method: 'DELETE' })

export const generateGoals = ({ sessionId, goal }: { sessionId: string; goal: string }) =>
...
export const createPublicShareLink = (sessionId: string, ttlDays: number, label?: string) => {
    const params = new URLSearchParams({ ttl_days: ttlDays.toString() });
    if (label) {
        params.append('label', label);
    }
    return apiFetch(`/public/share/session/${sessionId}?${params.toString()}`, { method: 'POST' });
};
...
export const bootstrapSharedSession = (shareToken: string, visitorId: string) =>
...
export const fetchFiles = (sessionId: string) => apiFetch(`/session/${sessionId}/outputs`)

export const fetchFileContent = async (sessionId: string | undefined, shareToken: string | undefined, visitorId: string | undefined, fileName: string) => {
    let path = '';
    if (shareToken && visitorId) {
        path = `/public/${shareToken}/files/${visitorId}/${encodeURIComponent(fileName)}`;
    } else if (sessionId) {
